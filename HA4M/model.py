...
class skeSubnet(nn.Module):
    ...
    def __init__(self,output_size):
        super(skeSubnet, self).__init__()
        self.feature=C3D(output_size)

    def forward(self, x):
        x=self.feature(x)
        return x

class inertialSubNet(nn.Module):  # color

    def __init__(self, hidden_size):

        super(inertialSubNet, self).__init__()
        self.feature=C3D(hidden_size)

    def forward(self, x):
        x=self.feature(x)
        return x
class depthSubnet(nn.Module): # colorA
    def __init__(self,out_size):
        super().__init__()
        self.features=C3D(out_size)
...
class BlockTrainerBlend(nn.Module):
    def __init__(self,args,dev,
        ...
        self.inertialSubnet = inertialSubNet(mm_dim) # color
        self.skeletonSubnet = skeSubnet( output_size=mm_dim) # skeleton
        self.depthSubnet=depthSubnet(mm_dim) # colorA
    ...
    def forward(self,  colorA,color,skeleton, subnet_flag,blockAtt,mode='train'): #[colorA,color,skeleton]
        # referClassifier=copy.deepcopy(self.linear_out).to(self.dev)
        # for param in referClassifier.parameters():
        #     param.requires_grad = False

        bsize = colorA.shape[0]

        x0 = self.depthSubnet(colorA) if subnet_flag[0]==1 else self.x_0
        x1 = self.inertialSubnet(color) if subnet_flag[1]==1 else self.x_1
...
class C3D(nn.Module):
    ...
    def __init__(self, num_classes):
        ...
        self.conv5a = nn.Conv3d(256,128, kernel_size=(3, 3, 3), padding=(1, 1, 1))
        ...
        self.fc6 = nn.Linear(int(8192/4), num_classes)
        # self.fc7 = nn.Linear(4096, 4096)
        # self.fc8 = nn.Linear(4096, num_classes)
        # self.fc7 = nn.Linear(1000, num_classes)
    ...
    def forward(self, x):
        ...
        x = x.view(-1, 128*1*4*4)
        # x = self.relu(self.fc6(x))
        # x = self.dropout(x)
        # logits = self.relu(self.fc7(x))
        logits = self.relu(self.fc6(x))
