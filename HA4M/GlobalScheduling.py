...
class GlobalSchedule(object):
    def __init__(self,args,data_loader_test,dataset_len,client_data,data_loader_valid,validData,trainData,client_data_frac):
        ...
        self.local_test_result = {
            key: []
            for key in ['CA', 'C', 'D', 'CA+C', 'CA+D', 'C+D', 'CA+C+D']
        }
        ...
        self.newest_globalModel=copy.deepcopy(self.globalModel.state_dict())
        ...
        if self.args.isAsync==2:
            self.participate_num=[0 for i in range(self.args.client_num)]
            self.dynamic_data_columns=['client_{}'.format(i) for i in range(self.args.client_num)]
            self.dynamic_result_columns=['benefitLoss','benefitAcc']
            self.dynamic_participated_client_data=[[1 for i in range(args.client_num)]]
            self.dynamic_participated_result=[[1,1]]
            self.selfinfo_export_list=[]
            self.selected_export_list=[]
            self.last_loss=100
            self.last_acc=0
        self.blockCorrect=[0.0 for i in range(self.args.chunks+1)]
        if self.args.isBlend==1:
    ...
    def GlobalIterate(self,global_epo):
        ...
        self.global_weights = copy.deepcopy(self.aggregate_op.asyncAggBlend(self.participating_client_subnet,self.local_weights,self.global_weights,self.clientTransIdx,self.clientOG,self.blockOG,self.args,self.clientValid,self.globalValid,self.clientglobalValid,self.local_grads,self.participating_client_list,self.client_global_weight,self.globalValidFlow,self.client_receive_global_timeFrame[client_idx],self.newest_valid,self.newest_time,self.deltaAccLocal,self.newest_globalModel,self.startAccLocal,self.args))
            # self.global_weights = self.aggregate_op.asyncAgg(self.participating_client_subnet,self.local_grads,self.global_weights) if self.args.isBlend == 0 else self.aggregate_op.asyncAggBlend(self.participating_client_subnet,self.local_grads,self.global_weights,self.clientTransIdx,self.clientOG,self.blockOG)

        # print training metric

        for idx in self.participating_client_list:
            if self.client_receive_global_timeFrame[idx]>=self.newest_time:
                self.newest_valid = self.clientglobalValid
                self.newest_globalModel = copy.deepcopy(self.client_global_weight[idx])
                self.newest_time = self.client_receive_global_timeFrame[idx]
        ...
        for subnet_flag_idx in subnet_list:
            ...
            with torch.no_grad():
                for _, data_batch in enumerate(self.data_loader_test):
                    ...
                    color, colorA, skeleton, labels = data_batch[0].cuda().float(), data_batch[1].cuda().float(),data_batch[2].cuda().float(),data_batch[3].cuda().long()

                    output,modalityOut=self.globalModel(colorA,color,skeleton, subnet_flag_idx,self.blockAtt,mode='test')
            ...
            if subnet_flag_idx[0] == 1:
                _subnet_str.append('CA')
            if subnet_flag_idx[1] == 1:
                _subnet_str.append('C')
            if subnet_flag_idx[2] == 1:
                _subnet_str.append('D')
    ...
    def LocalUpdateBlend(self,client_idx,pbar,clientOrderIdx,global_epo):
        ...
        self.clientglobalValid,localModelOgrTermStartValid=self.eval_step(eval_data_loader=self.clientValidData[client_idx],clientDataset=self.validData,subnetFlags=subnetFlag,model=self.client_global_weight[client_idx],client_subnet=self.client_subnet_flag[client_idx])
        self.globalValidFlow[global_epo]=self.clientglobalValid
        threshold = self.args.thred
        ...
        for loc_epo in range(self.args.local_ep):
            ...
            for _,data_batch in enumerate(self.client_data[client_idx]):
                batchNum+=1
                color=torch.tensor(self.trainData.color[data_batch[0]]).cuda().float()
                colorA=torch.tensor(self.trainData.colorA[data_batch[0]]).cuda().float()
                skeleton=torch.tensor(self.trainData.skeleton[data_batch[0]]).cuda().float()
                labels=torch.tensor(self.trainData.labels[data_batch[0]]).cuda().long()
                if len(data_batch[0]) == 1:
                    color = torch.unsqueeze(color, dim=0)
                    colorA = torch.unsqueeze(colorA, dim=0)
                    skeleton = torch.unsqueeze(skeleton, dim=0)
                    labels = torch.unsqueeze(labels, dim=0)

                self.optimizer.zero_grad()
                output, modalityOut = self.globalModel(colorA,color, skeleton,self.client_subnet_flag[client_idx], self.blockAtt)
            ...
            if loc_epo==0:
                ...
                _,localModelOgrTermEndValid=self.eval_step(eval_data_loader=self.clientValidData[client_idx],clientDataset=self.validData,subnetFlags=[self.client_subnet_flag[client_idx]],client_subnet=self.client_subnet_flag[client_idx])
                ...
                if loc_epo==int(self.args.local_ep-1):
                    self.clientValid, localModelOgrTermEndValid = self.eval_step(eval_data_loader=self.clientValidData[client_idx],clientDataset=self.validData,subnetFlags=subnetFlag,client_subnet=self.client_subnet_flag[client_idx])
                    ...
                    self.deltaAccLocal = abs(self.clientValid - self.clientglobalValid)


                    subnetFlag = [[1,0,0],[0,1,0],[0,0,1]] if (self.args.splitScheme.lower() in ['\'mix\'', 'mix']) and (self.args.subnet_num==1) else [[1,1,0],[0,1,1],[1,0,1]] if (self.args.splitScheme.lower() in ['\'mix\'', 'mix']) and (self.args.subnet_num==2) else [[1,1,1]] if (self.args.splitScheme.lower() in ['\'mix\'', 'mix']) and (self.args.subnet_num==3) else [self.client_subnet_flag[0]]
                    # t+T globalModel
                    self.globalModel.load_state_dict(self.global_weights)
                    self.globalValid,globalModelOgrTermStartValid=self.eval_step(eval_data_loader=self.clientValidData[client_idx],clientDataset=self.validData,model=self.global_weights,subnetFlags=subnetFlag,client_subnet=self.client_subnet_flag[client_idx])
                    self.globalValid=self.newest_valid

                    _,globalModelOgrTermStartTrain=self.eval_step(eval_data_loader=self.client_data[client_idx],clientDataset=self.trainData,model=self.global_weights,subnetFlags=[self.client_subnet_flag[client_idx]],client_subnet=self.client_subnet_flag[client_idx])
        ...
        for i in np.flatnonzero(np.array(self.client_subnet_flag[client_idx])==1):
            ...
            if len(modalityLayer)>0:
                ...
                for key in modalityLayer:
                    ...
                    tmp = tmp + torch.norm((local_module_wt - global_module_wt).float(), 2)
                modalityGrad[i]=tmp
            # modalityF1[i] = (2 * modalityGrad[i] / modalityOGR[i]) / (modalityGrad[i] + 1 / modalityOGR[i])
        ...
        for key in self.global_weights.keys():
            if 'Subnet' in key:
                ...
                if 'merge_linears' in key:
                    ...
                    if self.blockAtt[block_idx]>=0.5:
                        self.clientTransIdx[key]+=[clientOrderIdx]
                else:
                    self.clientTransIdx[key]+=[clientOrderIdx]
    ...
    def eval_step(self, eval_data_loader,clientDataset,model=None,subnetFlags=[[1,1,1]],type='acc',client_subnet=[1,1,1]):
        ...
        for subnetFlag in subnetFlags:
            ...
            for _, data_batch in enumerate(eval_data_loader):
                color=torch.tensor(clientDataset.color[data_batch[0]]).cuda().float()
                colorA=torch.tensor(clientDataset.colorA[data_batch[0]]).cuda().float()
                skeleton=torch.tensor(clientDataset.skeleton[data_batch[0]]).cuda().float()
                labels=torch.tensor(clientDataset.labels[data_batch[0]]).cuda().long()
                if len(data_batch[0]) == 1:
                    color = torch.unsqueeze(color, dim=0)
                    colorA = torch.unsqueeze(colorA, dim=0)
                    skeleton = torch.unsqueeze(skeleton, dim=0)
                ...
                output,modalityOut = self.globalModel(colorA, color,skeleton, subnetFlag,self.blockAtt,mode='test')
...
class ServerUpdate():
    ...
    def asyncAggBlend(self,participating_client_subnet,local_weights,server_global_weight,clientTransIdx,clientOG,blockOG,args,clientValid,globalValid,clientglobalValid,local_grads,participating_client_list,client_global_weight,globalValidFlow,client_receive_global_timeFrame,newest_valid,newest_time,deltaAccLocal,newest_globalModel,startAccLocal,arg):
        '''
        ...
        elta=math.pow(arg.p,abs(deltaO/(deltaG+1e-10)))
...
if __name__ == '__main__':
    ...
    server=GlobalSchedule(args,0,{'test':10},[[1,1]])
    print(server.aggregate_op.local_weights)
    server.local_weights.append(1)
    server.local_weights.append(2)
