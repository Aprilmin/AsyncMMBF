...
def get_optimal_gradient_blend_weights(modalities_losses_n,
    ...
    """
    modality_names = range(len(modalities_losses_n['eval']))
    Z = 0
    modls_GO = dict()
    ...
    optimal_weights = []
    for modality_nm in modality_names:
        # optimal_weights.append(modls_GO[modality_nm] / Z)
        optimal_weights.append(modls_GO[modality_nm]/np.sum(list(modls_GO.values())))
...
def get_optimal_gradient_blend_weights_og(delta_OGs):
    ...
    for cli_i in range(num_of_clients):
        ...
        G_i = cli_delta_G+1e-10
        O_i = cli_delta_O+1e-10
